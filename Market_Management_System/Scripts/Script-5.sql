INSERT INTO PAYMENT
(PAYMENTNUM, ONUM, SALEAMT, DISCOUNTAMT, PAYMENTAMT, PAYMENTDATE)
VALUES(PAYMENTNUM_SEQ.NEXTVAL, (SELECT ONUM FROM "ORDER" o  WHERE ONUM ='63') ,(SELECT ODSTOCK*OPRICE FROM ORDERDETAIL o WHERE ICODE = '1002'), 
(SELECT SUM(MMONEY) MMONEY FROM MILEAGE m)+(SELECT c.COUPON FROM COUPON c WHERE CSEQ = '41'), 
(SELECT ODSTOCK*OPRICE FROM ORDERDETAIL o WHERE ICODE = '1002')-((SELECT SUM(MMONEY) MMONEY FROM MILEAGE m)+(SELECT c.COUPON FROM COUPON c WHERE CSEQ = '41')), SYSDATE);


SELECT ONUM FROM "ORDER" o  WHERE ONUM ='63';
SELECT ODSTOCK*OPRICE FROM ORDERDETAIL o WHERE ICODE = '1002';

SELECT SUM(MMONEY) - '90' 
	FROM MILEAGE m 
	WHERE MSTATE = 'P' ;

SELECT c.COUPON FROM COUPON c WHERE CSEQ = '41';
	
SELECT *
	FROM MILEAGE m ;
	
DELETE FROM MARKET."ORDER" WHERE ONUM='75';
DELETE FROM PAYMENT p WHERE ONUM='75';
SELECT * FROM PAYMENT p ;
SELECT * FROM "ORDER" o ;
SELECT * FROM ORDERDETAIL o ;
DELETE FROM ORDERDETAIL o ;


-- 결제 완료 시 결제내역 페이지 추가
INSERT INTO PAYMENT
(PAYMENTNUM, ONUM, SALEAMT, DISCOUNTAMT, PAYMENTAMT, PAYMENTDATE)
VALUES(PAYMENTNUM_SEQ.NEXTVAL, (SELECT ONUM FROM "ORDER" o  WHERE ONUM ='63') ,(SELECT ODSTOCK*OPRICE FROM ORDERDETAIL o WHERE ICODE = '1002'), 
(SELECT MMONEY FROM MILEAGE m WHERE MSTATE ='M' AND MSEQ ='63')+(SELECT c.COUPON FROM COUPON c WHERE CSEQ = '41'), 
(SELECT ODSTOCK*OPRICE FROM ORDERDETAIL o WHERE ICODE = '1002')-((SELECT MMONEY FROM MILEAGE m WHERE MSTATE ='M' AND MSEQ ='63')+(SELECT c.COUPON FROM COUPON c WHERE CSEQ = '41')), SYSDATE);


INSERT INTO MILEAGE ( MSEQ,ID,MMONEY,MDATE,MSTATE)
VALUES(MSEQ_SEQ.NEXTVAL,'test01','20',SYSDATE,'M');

UPDATE MILEAGE m SET  m.MMONEY = (SELECT SUM(MMONEY) - '20' FROM MILEAGE m2) , m.ID ='test01';

INSERT INTO MILEAGE 

SELECT MMONEY FROM MILEAGE m WHERE MSTATE ='M' AND MSEQ ='63';

DELETE FROM PAYMENT WHERE PAYMENTNUM = '8';
DELETE FROM "ORDER" o WHERE ONUM ='82';


SELECT TO_DATE(ODATE) ODATE , o.ONUM ,i.INAME ,
	   o2.ODSTOCK ,o2.OPRICE ,o.STORE ,
	   o.OSTATE ,m.NAME ,o.ADDR , 
	   m.PHONE , p.PAYMENTAMT , p.DISCOUNTAMT 
		FROM "ORDER" o 
		INNER JOIN ORDERDETAIL o2 ON o2.ONUM = o.ONUM 
		INNER JOIN INVENTORY i ON i.ICODE = o2.ICODE 
		INNER JOIN "MEMBER" m ON m.ID = o.ID 
		INNER JOIN PAYMENT p ON p.ONUM = o.ONUM 
		WHERE o.ONUM = '63';
		
SELECT * FROM "ORDER" o WHERE ONUM = '78';

UPDATE MILEAGE 
	SET MMONEY = (SELECT PAYMENTAMT FROM PAYMENT WHERE PAYMENTNUM = '6'), MSTATE = 'P', MDATE = SYSDATE 
	WHERE MSEQ = '63';

SELECT *FROM MILEAGE m ;


SELECT TO_DATE(ODATE) ODATE , o2.ODNUM ,i.INAME ,
	   o2.ODSTOCK ,o2.OPRICE ,o.STORE ,
	   o.OSTATE ,m.NAME ,o.ADDR , 
	   m.PHONE , p.PAYMENTAMT , p.DISCOUNTAMT 
		FROM "ORDER" o 
		INNER JOIN ORDERDETAIL o2 ON o2.ONUM = o.ONUM 
		INNER JOIN INVENTORY i ON i.ICODE = o2.ICODE 
		INNER JOIN "MEMBER" m ON m.ID = o.ID 
		INNER JOIN PAYMENT p ON p.ONUM = o.ONUM 
		WHERE o.ONUM = '63';
	
	
SELECT *
	FROM MILEAGE m ;

INSERT INTO MILEAGE (MSEQ ,ID ,MMONEY, MDATE,MSTATE)
	VALUES()
	
	
	SELECT DISTINCT 
			(SELECT SUM(MMONEY) MMONEY FROM MILEAGE WHERE ID = 'test01' AND MSTATE = 'P') - 
			(SELECT SUM(MMONEY) MMONEY FROM MILEAGE WHERE ID = 'test01' AND MSTATE = 'M') MMONEY 
				FROM MILEAGE m;
	
	SELECT COUNT(ID) ID
		FROM "MEMBER" m
		WHERE ID = 'test05';
	
SELECT *	
	FROM PAYMENT p ;

INSERT INTO PAYMENT (PAYMENTNUM,ONUM,SALEAMT,DISCOUNTAMT,PAYMENTAMT,PAYMENTDATE)
VALUES(PAYMENTNUM_SEQ.NEXTVAL,'81','5000','2000','3000',SYSDATE);

SELECT ONUM, ID ,STORE ,ODATE ,ROADADDR ,ADDR ,DELIVERYSELECT ,OSTATE 
	FROM "ORDER" o 
	WHERE ONUM ='81';

UPDATE "ORDER" SET ODATE = SYSDATE , OSTATE ='B' WHERE ONUM ='83';

SELECT TO_DATE(ODATE) ODATE , o2.ODNUM ,i.INAME ,
	   o2.ODSTOCK ,o2.OPRICE ,o.STORE ,
	   o.OSTATE ,m.NAME ,o.ADDR , 
	   m.PHONE , p.PAYMENTAMT , p.DISCOUNTAMT 
		FROM "ORDER" o 
		INNER JOIN ORDERDETAIL o2 ON o2.ONUM = o.ONUM 
		INNER JOIN INVENTORY i ON i.ICODE = o2.ICODE 
		INNER JOIN "MEMBER" m ON m.ID = o.ID 
		LEFT JOIN PAYMENT p ON p.ONUM = o.ONUM 
		WHERE o.ONUM = '83';
	
	
	SELECT TO_DATE(ODATE)ODATE , o.ONUM ,i.INAME ,o2.ODSTOCK ,o2.OPRICE ,o.STORE ,o.OSTATE 
	FROM "ORDER" o 
	INNER JOIN ORDERDETAIL o2 ON o2.ONUM = o.ONUM 
	INNER JOIN INVENTORY i ON i.ICODE = o2.ICODE 
	INNER JOIN "MEMBER" m ON m.ID = o.ID 
	WHERE m.ID ='test01'  AND o.OSTATE IN ('P','H','F','D','R')
	ORDER BY o.ODATE DESC ;


INSERT INTO MILEAGE (MSEQ,ID,MMONEY,MDATE,MSTATE)
		VALUES(MSEQ_SEQ.NEXTVAL,'test01',(SELECT FLOOR(PAYMENTAMT*0.01) FROM PAYMENT WHERE PAYMENTNUM = '97'),SYSDATE, 'P');
		
		
SELECT *
FROM MILEAGE p ;

SELECT DISTINCT 
			(SELECT SUM(MMONEY) MMONEY FROM MILEAGE WHERE ID = 'test01' AND MSTATE = 'P') - 
			(SELECT SUM(MMONEY) MMONEY FROM MILEAGE WHERE ID = 'test01' AND MSTATE = 'M') MMONEY 
				FROM MILEAGE m
				WHERE m.ID = 'test01'
				ORDER BY m.M;
			
			SELECT MSEQ ,ID ,MMONEY ,MDATE ,MSTATE 
		FROM MILEAGE m 
		WHERE ID ='test01'
		ORDER BY MDATE DESC;
	
	
	
	
	SELECT ODATE , o.ONUM ,i.INAME ,o2.ODSTOCK ,o2.OPRICE ,o.STORE ,o.OSTATE , o.ROADADDR , o.ADDR ,m.PHONE 
	FROM "ORDER" o 
	INNER JOIN ORDERDETAIL o2 ON o2.ONUM = o.ONUM 
	INNER JOIN INVENTORY i ON i.ICODE = o2.ICODE 
	INNER JOIN "MEMBER" m ON m.ID = o.ID 
	WHERE m.ID ='test01'  AND o.OSTATE IN ('P','H','F','D','R')
	ORDER BY o.ODATE DESC;

UPDATE MILEAGE 
		SET MMONEY = (SELECT PAYMENTAMT FROM PAYMENT WHERE PAYMENTNUM = '24'), MSTATE = 'R', MDATE = SYSDATE 
		WHERE MSEQ = '82';
		
	

CREATE TABLE T1(
   COL1 VARCHAR2(20),
   COL2 VARCHAR2(20)
);
INSERT INTO T1 VALUES('COL1_V1','COL2_V2');
INSERT INTO T1 VALUES('COL1_V2',NULL);
INSERT INTO T1 VALUES('COL1_V3','COL2_V3');

CREATE TABLE T2(
   COL3 VARCHAR2(20),
   COL4 VARCHAR2(20)
);
INSERT INTO T2 VALUES(NULL,'COL1_V1');
INSERT INTO T2 VALUES('COL3_V2','COL1_V2');
INSERT INTO T2 VALUES('COL1_V3','COL1_V3');

SELECT s2.STADIUM_NAME , t.TEAM_NAME , t2.TEAM_NAME ,HOME_SCORE , AWAY_SCORE 
   FROM SCHEDULE s 


SELECT s2.STADIUM_NAME , t.TEAM_NAME , t2.TEAM_NAME ,HOME_SCORE , AWAY_SCORE 
   FROM SCHEDULE s 
   JOIN STADIUM s2 ON s.STADIUM_ID = s2.STADIUM_ID
   JOIN TEAM t ON s.HOMETEAM_ID = t.TEAM_ID 
   JOIN TEAM t2 ON s.AWAYTEAM_ID = t2.TEAM_ID 
   WHERE HOME_SCORE >= AWAY_SCORE +3 ;












	
	
	
	
	
	
	